      subroutine rhs_xy_lin(hp1,hp1_pascal,rhshpxy1,rhshmxy1,rhspascalxy1,
     ...
     &     psieqx, phieqx, cureqx, hpeqx, hmeqx,
     &     tparex,tperpex,
     &     tparey,tperpey,
     &     pascalx,pascaly,
     &     qparex,qperpex,
...
c     derivo (rispetto a x)
      ...
      do iz = 1,nzl
         do iy = 1,nyl
            ...
            do ix = 1,nx
               phix(ix,iy,iz) = d1(ix)
               phieqx(ix,iy,iz)= phieq/(dcosh(x(ix)))**2.0d0
               psix(ix,iy,iz) = d2(ix) 
               psieqx(ix,iy,iz) = - psoeq * dtanh(x(ix)/eq_l)/eq_l
     &                 + asym*yl/zl
               curx(ix,iy,iz) = dd1(ix) 
               cureqx(ix,iy,iz) = - psoeq  
     &          * 2.0d0 * dtanh(x(ix)/eq_l)
     &          *(1.0d0 - (dtanh(x(ix)/eq_l))**2.0d0)/(eq_l**3.)
               hpx(ix,iy,iz) = dd2(ix) 
               hpeqx(ix,iy,iz) = - psoeq * dtanh(x(ix)/eq_l)/eq_l
     &              + asym*yl/zl
     &          - de2 * psoeq * 2.0d0 * dtanh(x(ix)/eq_l) * 
     &          (1.0d0 - dtanh(x(ix)/eq_l)*dtanh(x(ix)/eq_l))
     &         /(eq_l**3.) 
               hmx(ix,iy,iz) = ddd1(ix) 
               hmeqx(ix,iy,iz) = - 2.0d0*phieq
...
!-------------------------------------------------
        ...
        rhshpxy1 = - (phieqx * hpy - phiy * hpeqx)
     &             - (0.4d0*0.4d0) * (psieqx * hmy - psiy * hmeqx)
     &             - 0.0d0 * (psix * tparey - psiy * tparex)
     &             - eta * cur_old

        rhshmxy1 = - (phieqx * hmy - phiy * hmeqx)
     &             - 2./beta_e * (psieqx * cury - psiy * cureqx)
